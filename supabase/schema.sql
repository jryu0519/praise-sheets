...
-- A dedicated bucket (not the shared 'charts' bucket) keeps the privacy
-- story simple: ownership is just the storage path's leading folder
-- (`${userId}/${filename}`), no need to carve exceptions into the existing
